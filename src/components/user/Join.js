...
const Join = () => {
  ...
  const nameHandler = (e) => {
    ...
    let flag = false; // 입력값 검증 여부 체크 함수
  ...
  const fetchSignupPost = async () => {
    ...
    */
    ...
    if (res.status === 200) {
      ...
      alert(
        `${data.userName}(${data.eamil})님 회원가입에 성공했습니다.`,
      );
      // 로그인 페이지로 리다이렉트
      redirection('/login');
    } else {
  ...
  // 이미지 파일을 선택했을 때 썸네일을 뿌려주는 이벤트 핸들러
  const showThumbnailHandler = (e) => {
    ...
    if (
      ...
      alert(
        '이미지 파일(jpa, png, jpeg, gif)만 등록이 가능합니다.',
      );
      // 형식에 맞지 않는 파일을 첨부한 것이 파악된다면,input의 상태도 원래대로 돌려놓아야 한다.
